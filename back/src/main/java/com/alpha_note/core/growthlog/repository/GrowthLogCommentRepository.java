...
public interface GrowthLogCommentRepository extends JpaRepository<GrowthLogComment, Long> {
    ...
    // 댓글 수 카운트
    long countByGrowthLogEntity_IdAndIsDeletedFalse(Long growthLogId);

    // 특정 성장기록의 모든 댓글 조회 (삭제된 것 포함 - Soft Delete 처리용)
    List<GrowthLogComment> findByGrowthLogEntity_Id(Long growthLogId);

    // 성장기록별 댓글 조회 (생성일 내림차순 - 최신순)
    List<GrowthLogComment> findByGrowthLogEntity_IdAndIsDeletedFalseOrderByCreatedAtDesc(Long growthLogId);
}
